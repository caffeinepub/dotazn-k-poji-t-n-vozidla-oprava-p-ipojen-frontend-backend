...
export default function InsuranceFormComponent() {
  ...
  const [provozovatelExecutiveName, setProvozovatelExecutiveName] =
    useState("");
  const [provozovatelExecutivePhone, setProvozovatelExecutivePhone] =
    useState("");
  const [provozovatelExecutiveEmail, setProvozovatelExecutiveEmail] =
    useState("");
  const [provozovatelExecutiveRodneCislo, setProvozovatelExecutiveRodneCislo] =
    useState("");
  const [provozovatelCompanyFirstName, setProvozovatelCompanyFirstName] =
    useState("");
  const [provozovatelCompanyLastName, setProvozovatelCompanyLastName] =
    useState("");
  ...
  const [pojistnikExecutiveRodneCislo, setPojistnikExecutiveRodneCislo] =
    useState("");
  const [pojistnikCompanyFirstName, setPojistnikCompanyFirstName] =
    useState("");
  ...
  const [vlastnikExecutiveRodneCislo, setVlastnikExecutiveRodneCislo] =
    useState("");
  ...
  const [isSameAsProvozovatelPojistnik, setIsSameAsProvozovatelPojistnik] =
    useState(false);
  ...
  useEffect(() => {
    ...
    provozovatelEmail,
    provozovatelIco,
    provozovatelRodneCislo,
    provozovatelIsCompany,
    provozovatelIsVatPayer,
    provozovatelExecutiveName,
    provozovatelExecutivePhone,
    provozovatelExecutiveEmail,
    provozovatelExecutiveRodneCislo,
    provozovatelCompanyFirstName,
    provozovatelCompanyLastName,
  ]);
  ...
  // Vozidlo - New fields
  const [hasTechnicalCertificate, _setHasTechnicalCertificate] =
    useState(false);
  ...
  const [pojisteniPrirodnichRizik, setPojisteniPrirodnichRizik] =
    useState(false);
  const [pojisteniGAP, setPojisteniGAP] = useState(false);
  const [pojisteniNahradnihoVozidla, setPojisteniNahradnihoVozidla] =
    useState(false);
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: brand triggers model reset
  useEffect(() => {
    setModel("");
  }, [brand]);

  // Auto-expand textarea for notes
  // biome-ignore lint/correctness/useExhaustiveDependencies: notes triggers textarea resize
  useEffect(() => {
  ...
  const validateForm = (): ValidationError[] => {
    ...
    if (provozovatelIsCompany) {
      ...
      if (!provozovatelAdresa.trim()) {
        errors.push({
          field: "provozovatelAdresa",
          message: "Trvalá adresa je povinná",
          section: "Provozovatel",
        });
      }
    }
    ...
    if (!povinneRuceni && !havarijniPojisteni) {
      errors.push({
        field: "insuranceOptions",
        message: "Vyberte alespoň jednu možnost",
        section: "Požadované pojištění",
      });
    }
    ...
    if (havarijniPojisteni && !mileage.trim()) {
      errors.push({
        field: "mileage",
        message: "Stav tachometru je povinný při havarijním pojištění",
        section: "Požadované pojištění",
      });
    }
    ...
    if (importedFromAbroad) {
      if (!vehicleTypeSelected) {
        errors.push({
          field: "vehicleTypeSelected",
          message: "Druh vozidla je povinný",
          section: "Údaje o vozidle",
        });
      }
      if (!vin.trim()) {
        errors.push({
          field: "vin",
          message: "VIN je povinný",
          section: "Údaje o vozidle",
        });
      }
      ...
      if (!brand.trim()) {
        errors.push({
          field: "brand",
          message: "Tovární značka je povinná",
          section: "Údaje o vozidle",
        });
      }
      if (!model.trim()) {
        errors.push({
          field: "model",
          message: "Modelová řada je povinná",
          section: "Údaje o vozidle",
        });
      }
      if (!engineCapacity.trim()) {
        errors.push({
          ...
          message: "Zdvihový objem je povinný",
          section: "Údaje o vozidle",
        });
      }
      if (!maxPower.trim()) {
      ...
      if (!approximateValue.trim()) {
        errors.push({
          ...
          message: "Přibližná hodnota vozidla je povinná",
          section: "Údaje o vozidle",
        });
      }
      if (!fuelType) {
    ...
    if (!paymentFrequency) {
      errors.push({
        ...
        message: "Vyberte frekvenci plateb",
        section: "Sekvence plateb",
      });
    }
  ...
  const handleInitialSubmit = () => {
    ...
    if (errors.length > 0) {
      ...
      toast.error(
        `Formulář obsahuje ${errors.length} chyb. Prosím opravte označená pole.`,
        {
          duration: 5000,
        },
      );
  ...
  const handleFinalSubmit = async (
    ...
    const form: InsuranceForm = {
      ...
      provozovatel: {
        ...
        phone: provozovatelIsCompany
          ? provozovatelExecutivePhone
          : provozovatelTelefon,
        email: provozovatelIsCompany
          ? provozovatelExecutiveEmail
          : provozovatelEmail,
        ico: provozovatelIco,
    ...
    try {
      ...
      toast.error(
        "Chyba při odesílání formuláře. Zkontrolujte připojení k backendu.",
        {
          duration: 5000,
        },
      );
  ...
  const hasError = (fieldName: string) => {
    return (
      ...
      validationErrors.some((err) => err.field === fieldName)
    );
  };
  ...
  const hasInsuranceOptionsError = () => {
    return (
      showValidationErrors &&
      validationErrors.some((err) => err.field === "insuranceOptions")
    );
  };
  ...
  const UnifiedToggle = ({
    ...
    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-3 bg-gradient-to-br from-gray-50 to-white p-2.5 rounded-lg shadow-sm border border-gray-200 hover:shadow-md transition-all duration-300">
      <Label className="text-xs font-semibold text-gray-700 flex-1 min-w-0 leading-relaxed">
        {label}
      </Label>
      <div className="flex items-center justify-end sm:justify-start space-x-2 flex-shrink-0 ml-auto">
        <span
          className={`text-xs font-medium transition-all duration-300 whitespace-nowrap ${!checked ? "text-gray-900 scale-105" : "text-gray-400"}`}
        >
          Ne
        </span>
        <button
        ...
        <span
          className={`text-xs font-medium transition-all duration-300 whitespace-nowrap ${checked ? "text-red-700 scale-105 font-bold" : "text-gray-400"}`}
        >
          Ano
        </span>
      </div>
      ...
      <div className="max-w-4xl mx-auto text-center py-16">
        <div className="bg-gradient-to-br from-white to-gray-50 rounded-2xl shadow-2xl p-12 border border-gray-200 transform hover:scale-105 transition-transform duration-300">
          ...
          <p className="text-lg text-gray-600 mb-8">
            Děkujeme za vyplnění dotazníku pojištění vozidla. Váš formulář byl
            úspěšně doručen a brzy se vám ozveme.
          </p>
          <Button
            onClick={resetForm}
            size="lg"
      ...
      <div className="max-w-5xl mx-auto">
              ...
              <div className="flex-1">
                ...
                <div className="space-y-2">
                  {validationErrors.map((error, index) => (
                    <div
                      key={`${error.section}-${error.message}-${index}`}
        ...
        <div className="bg-white shadow-2xl rounded-2xl overflow-hidden border border-gray-200">
          <div className="px-8 py-8 space-y-8">
              ...
              <div className="flex items-center space-x-3">
                <Checkbox
                  ...
                  onCheckedChange={(checked) =>
                    setSameOperator(checked as boolean)
                  }
                  className="border-white data-[state=checked]:bg-white data-[state=checked]:text-red-800 h-5 w-5"
                />
                <Label
                  htmlFor="sameOperator"
                  className="text-sm font-bold cursor-pointer text-white uppercase leading-tight"
                >
            ...
            {/* BOD 1: PROVOZOVATEL */}
            <div
              className={`bg-white rounded-2xl shadow-lg border-2 p-8 transition-all duration-300 ${hasError("provozovatelJmeno") || hasError("provozovatelAdresa") || hasError("provozovatelIco") || hasError("provozovatelRodneCislo") ? "border-red-500 animate-errorBlink" : "border-gray-200"}`}
              data-error={
                  ...
                  <>
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        className={
                          hasError("provozovatelIco") ? "border-red-500" : ""
                        }
                      />
                      {hasError("provozovatelIco") && (
                        <p className="text-xs text-red-600">
                          {getErrorMessage("provozovatelIco")}
                        </p>
                      )}
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        <Label
                          htmlFor="provozovatelCompanyFirstName"
                          className="text-sm font-medium text-gray-700"
                        >
                          Jméno
                        </Label>
                        <Input
                          ...
                          onChange={(e) =>
                            setProvozovatelCompanyFirstName(e.target.value)
                          }
                      ...
                      <div className="space-y-2">
                        <Label
                          htmlFor="provozovatelCompanyLastName"
                          className="text-sm font-medium text-gray-700"
                        >
                          Příjmení
                        </Label>
                        <Input
                          ...
                          onChange={(e) =>
                            setProvozovatelCompanyLastName(e.target.value)
                          }
                    ...
                    <div className="space-y-2">
                      ...
                      {hasError("provozovatelAdresa") && (
                        <p className="text-xs text-red-600">
                          {getErrorMessage("provozovatelAdresa")}
                        </p>
                      )}
                    </div>
                    ...
                    <div className="mt-6 pt-6 border-t border-gray-200">
                      <h3 className="text-sm font-semibold text-gray-900 mb-4">
                        Kontaktní osoba / Jednatel
                      </h3>

                      <div className="space-y-4">
                        <div className="space-y-2">
                          <Label
                            htmlFor="provozovatelExecutiveName"
                            className="text-sm font-medium text-gray-700"
                          >
                            Jméno a příjmení
                          </Label>
                          <Input
                            ...
                            onChange={(e) =>
                              setProvozovatelExecutiveName(e.target.value)
                            }
                        ...
                        <div className="space-y-2">
                          <Label
                            htmlFor="provozovatelExecutiveRodneCislo"
                            className="text-sm font-medium text-gray-700"
                          >
                            Rodné číslo
                          </Label>
                          <Input
                            ...
                            onChange={(e) =>
                              setProvozovatelExecutiveRodneCislo(e.target.value)
                            }
                        ...
                        <div className="space-y-2">
                          <Label
                            htmlFor="provozovatelExecutivePhone"
                            className="text-sm font-medium text-gray-700"
                          >
                            Telefon
                          </Label>
                          <Input
                            ...
                            onChange={(e) =>
                              setProvozovatelExecutivePhone(e.target.value)
                            }
                        ...
                        <div className="space-y-2">
                          <Label
                            htmlFor="provozovatelExecutiveEmail"
                            className="text-sm font-medium text-gray-700"
                          >
                            E-mail
                          </Label>
                          <Input
                            ...
                            onChange={(e) =>
                              setProvozovatelExecutiveEmail(e.target.value)
                            }
                  ...
                  <>
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        placeholder="Zadejte jméno a příjmení"
                        className={
                          hasError("provozovatelJmeno") ? "border-red-500" : ""
                        }
                      />
                      {hasError("provozovatelJmeno") && (
                        <p className="text-xs text-red-600">
                          {getErrorMessage("provozovatelJmeno")}
                        </p>
                      )}
                    </div>
                    ...
                    <div className="space-y-2">
                      <Label
                        htmlFor="provozovatelRodneCislo"
                        className="text-sm font-medium text-gray-700"
                      >
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setProvozovatelRodneCislo(e.target.value)
                        }
                        placeholder="Zadejte rodné číslo"
                        className={
                          hasError("provozovatelRodneCislo")
                      ...
                      {hasError("provozovatelRodneCislo") && (
                        <p className="text-xs text-red-600">
                          {getErrorMessage("provozovatelRodneCislo")}
                        </p>
                      )}
                    </div>
            ...
            {!sameOperator && (
              <div
                className={`bg-white rounded-2xl shadow-lg border-2 p-8 transition-all duration-300 ${hasError("pojistnikJmeno") || hasError("pojistnikAdresa") || hasError("pojistnikIco") || hasError("pojistnikRodneCislo") ? "border-red-500 animate-errorBlink" : "border-gray-200"}`}
                data-error={
                  ...
                  <div className="bg-gradient-to-r from-red-800 via-red-700 to-black p-4 rounded-xl shadow-md">
                    <div className="flex items-center space-x-3">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) =>
                          setIsSameAsProvozovatelPojistnik(checked as boolean)
                        }
                        className="border-white data-[state=checked]:bg-white data-[state=checked]:text-red-800 h-5 w-5"
                      />
                      <Label
                        htmlFor="isSameAsProvozovatelPojistnik"
                        className="text-sm font-bold cursor-pointer text-white uppercase leading-tight"
                      >
                      ...
                      {pojistnikIsCompany ? (
                        <>
                          <div className="space-y-2">
                            ...
                            <Input
                              ...
                              placeholder="Zadejte IČO"
                              className={
                                hasError("pojistnikIco") ? "border-red-500" : ""
                              }
                            />
                            {hasError("pojistnikIco") && (
                              <p className="text-xs text-red-600">
                                {getErrorMessage("pojistnikIco")}
                              </p>
                            )}
                          </div>
                          ...
                          <div className="space-y-2">
                            ...
                            <Input
                              ...
                              onChange={(e) =>
                                setPojistnikJmeno(e.target.value)
                              }
                              placeholder="Zadejte název firmy"
                            />
                          </div>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-2">
                              <Label
                                htmlFor="pojistnikCompanyFirstName"
                                className="text-sm font-medium text-gray-700"
                              >
                                Jméno
                              </Label>
                              <Input
                                ...
                                onChange={(e) =>
                                  setPojistnikCompanyFirstName(e.target.value)
                                }
                                placeholder="Zadejte jméno"
                              />
                            </div>
                            ...
                            <div className="space-y-2">
                              <Label
                                htmlFor="pojistnikCompanyLastName"
                                className="text-sm font-medium text-gray-700"
                              >
                                Příjmení
                              </Label>
                              <Input
                                ...
                                onChange={(e) =>
                                  setPojistnikCompanyLastName(e.target.value)
                                }
                                placeholder="Zadejte příjmení"
                              />
                            </div>
                          ...
                          <div className="space-y-2">
                            ...
                            {hasError("pojistnikAdresa") && (
                              <p className="text-xs text-red-600">
                                {getErrorMessage("pojistnikAdresa")}
                              </p>
                            )}
                          </div>
                          ...
                          <div className="mt-6 pt-6 border-t border-gray-200">
                            ...
                            <div className="space-y-4">
                              <div className="space-y-2">
                                <Label
                                  htmlFor="pojistnikExecutiveName"
                                  className="text-sm font-medium text-gray-700"
                                >
                                  Jméno a příjmení
                                </Label>
                                <Input
                                  ...
                                  onChange={(e) =>
                                    setPojistnikExecutiveName(e.target.value)
                                  }
                                  placeholder="Zadejte jméno a příjmení"
                                />
                              </div>
                              ...
                              <div className="space-y-2">
                                <Label
                                  htmlFor="pojistnikExecutiveRodneCislo"
                                  className="text-sm font-medium text-gray-700"
                                >
                                  Rodné číslo
                                </Label>
                                <Input
                                  ...
                                  onChange={(e) =>
                                    setPojistnikExecutiveRodneCislo(
                                      e.target.value,
                                    )
                                  }
                                  placeholder="Zadejte rodné číslo"
                              ...
                              <div className="space-y-2">
                                <Label
                                  htmlFor="pojistnikExecutivePhone"
                                  className="text-sm font-medium text-gray-700"
                                >
                                  Telefon
                                </Label>
                                <Input
                                  ...
                                  onChange={(e) =>
                                    setPojistnikExecutivePhone(e.target.value)
                                  }
                                  placeholder="Zadejte telefonní číslo"
                                />
                              </div>
                              ...
                              <div className="space-y-2">
                                <Label
                                  htmlFor="pojistnikExecutiveEmail"
                                  className="text-sm font-medium text-gray-700"
                                >
                                  E-mail
                                </Label>
                                <Input
                                  ...
                                  onChange={(e) =>
                                    setPojistnikExecutiveEmail(e.target.value)
                                  }
                                  placeholder="Zadejte e-mailovou adresu"
                                />
                              </div>
                        ...
                        <>
                          <div className="space-y-2">
                            ...
                            {hasError("pojistnikJmeno") && (
                              <p className="text-xs text-red-600">
                                {getErrorMessage("pojistnikJmeno")}
                              </p>
                            )}
                          </div>
                          ...
                          <div className="space-y-2">
                            ...
                            <Input
                              ...
                              onChange={(e) =>
                                setPojistnikRodneCislo(e.target.value)
                              }
                              placeholder="Zadejte rodné číslo"
                              className={
                                hasError("pojistnikRodneCislo")
                            ...
                            {hasError("pojistnikRodneCislo") && (
                              <p className="text-xs text-red-600">
                                {getErrorMessage("pojistnikRodneCislo")}
                              </p>
                            )}
                          </div>
                          ...
                          <div className="space-y-2">
                            ...
                            <Input
                              ...
                              onChange={(e) =>
                                setPojistnikTelefon(e.target.value)
                              }
                              placeholder="Zadejte telefonní číslo"
                            />
                          </div>
                          ...
                          <div className="space-y-2">
                            ...
                            <Input
                              ...
                              onChange={(e) =>
                                setPojistnikEmail(e.target.value)
                              }
                              placeholder="Zadejte e-mailovou adresu"
                            />
                          </div>
            ...
            {!sameOperator && (
              <div
                className={`bg-white rounded-2xl shadow-lg border-2 p-8 transition-all duration-300 ${hasError("vlastnikJmeno") || hasError("vlastnikAdresa") || hasError("vlastnikIco") || hasError("vlastnikRodneCislo") ? "border-red-500 animate-errorBlink" : "border-gray-200"}`}
                data-error={
                    ...
                    <>
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          placeholder="Zadejte IČO"
                          className={
                            hasError("vlastnikIco") ? "border-red-500" : ""
                          }
                        />
                        {hasError("vlastnikIco") && (
                          <p className="text-xs text-red-600">
                            {getErrorMessage("vlastnikIco")}
                          </p>
                        )}
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-2">
                          <Label
                            htmlFor="vlastnikCompanyFirstName"
                            className="text-sm font-medium text-gray-700"
                          >
                            Jméno
                          </Label>
                          <Input
                            ...
                            onChange={(e) =>
                              setVlastnikCompanyFirstName(e.target.value)
                            }
                            placeholder="Zadejte jméno"
                          />
                        </div>
                        ...
                        <div className="space-y-2">
                          <Label
                            htmlFor="vlastnikCompanyLastName"
                            className="text-sm font-medium text-gray-700"
                          >
                            Příjmení
                          </Label>
                          <Input
                            ...
                            onChange={(e) =>
                              setVlastnikCompanyLastName(e.target.value)
                            }
                            placeholder="Zadejte příjmení"
                          />
                        </div>
                      ...
                      <div className="space-y-2">
                        ...
                        {hasError("vlastnikAdresa") && (
                          <p className="text-xs text-red-600">
                            {getErrorMessage("vlastnikAdresa")}
                          </p>
                        )}
                      </div>
                        ...
                        <div className="space-y-4">
                          <div className="space-y-2">
                            ...
                            <Input
                              ...
                              onChange={(e) =>
                                setVlastnikExecutiveName(e.target.value)
                              }
                              placeholder="Zadejte jméno a příjmení"
                            />
                          </div>
                          ...
                          <div className="space-y-2">
                            <Label
                              htmlFor="vlastnikExecutiveRodneCislo"
                              className="text-sm font-medium text-gray-700"
                            >
                              Rodné číslo
                            </Label>
                            <Input
                              ...
                              onChange={(e) =>
                                setVlastnikExecutiveRodneCislo(e.target.value)
                              }
                              placeholder="Zadejte rodné číslo"
                            />
                          </div>
                          ...
                          <div className="space-y-2">
                            <Label
                              htmlFor="vlastnikExecutivePhone"
                              className="text-sm font-medium text-gray-700"
                            >
                              Telefon
                            </Label>
                            <Input
                              ...
                              onChange={(e) =>
                                setVlastnikExecutivePhone(e.target.value)
                              }
                              placeholder="Zadejte telefonní číslo"
                            />
                          </div>
                          ...
                          <div className="space-y-2">
                            <Label
                              htmlFor="vlastnikExecutiveEmail"
                              className="text-sm font-medium text-gray-700"
                            >
                              E-mail
                            </Label>
                            <Input
                              ...
                              onChange={(e) =>
                                setVlastnikExecutiveEmail(e.target.value)
                              }
                              placeholder="Zadejte e-mailovou adresu"
                            />
                          </div>
                    ...
                    <>
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          placeholder="Zadejte jméno a příjmení"
                          className={
                            hasError("vlastnikJmeno") ? "border-red-500" : ""
                          }
                        />
                        {hasError("vlastnikJmeno") && (
                          <p className="text-xs text-red-600">
                            {getErrorMessage("vlastnikJmeno")}
                          </p>
                        )}
                      </div>
                      ...
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setVlastnikRodneCislo(e.target.value)
                          }
                          placeholder="Zadejte rodné číslo"
                          className={
                            hasError("vlastnikRodneCislo")
                        ...
                        {hasError("vlastnikRodneCislo") && (
                          <p className="text-xs text-red-600">
                            {getErrorMessage("vlastnikRodneCislo")}
                          </p>
                        )}
                      </div>
            ...
            <div
              className={`bg-white rounded-2xl shadow-lg border-2 p-8 transition-all duration-300 ${hasInsuranceOptionsError() || hasError("mileage") ? "border-red-500 animate-errorBlink" : "border-gray-200"}`}
              data-error={hasInsuranceOptionsError() || hasError("mileage")}
            >
                ...
                {hasInsuranceOptionsError() && (
                  <div className="bg-red-50 border border-red-300 rounded-lg p-3">
                    <p className="text-sm text-red-700 font-medium">
                      {getErrorMessage("insuranceOptions")}
                    </p>
                  </div>
                )}
            ...
            <div
              className={`bg-white rounded-2xl shadow-lg border-2 p-8 transition-all duration-300 ${hasError("spz") || hasError("vin") || hasError("vehicleTypeSelected") || hasError("brand") || hasError("model") || hasError("engineCapacity") || hasError("maxPower") || hasError("weight") || hasError("approximateValue") || hasError("fuelType") ? "border-red-500 animate-errorBlink" : "border-gray-200"}`}
              data-error={
                hasError("spz") ||
                hasError("vin") ||
              ...
              <div className="space-y-4">
                  ...
                  <>
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onValueChange={setVehicleTypeSelected}
                      >
                        <SelectTrigger
                          className={`bg-white ${hasError("vehicleTypeSelected") ? "border-red-500" : ""}`}
                        >
                      ...
                      {hasError("vehicleTypeSelected") && (
                        <p className="text-xs text-red-600">
                          {getErrorMessage("vehicleTypeSelected")}
                        </p>
                      )}
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <Select value={fuelType} onValueChange={setFuelType}>
                        <SelectTrigger
                          className={`bg-white ${hasError("fuelType") ? "border-red-500" : ""}`}
                        >
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        className={
                          hasError("engineCapacity") ? "border-red-500" : ""
                        }
                      />
                      {hasError("engineCapacity") && (
                        <p className="text-xs text-red-600">
                          {getErrorMessage("engineCapacity")}
                        </p>
                      )}
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        className={
                          hasError("approximateValue") ? "border-red-500" : ""
                        }
                      />
                      {hasError("approximateValue") && (
                        <p className="text-xs text-red-600">
                          {getErrorMessage("approximateValue")}
                        </p>
                      )}
                    </div>
                  </>
                ...
                <div className="space-y-2">
                  ...
                  >
                    ...
                    <SelectContent className="bg-white">
                      <SelectItem value={UsageType.beznyProvoz}>
                        Běžný provoz
                      </SelectItem>
                      <SelectItem value={UsageType.sPravemPrednostniJizdy}>
                        S právem přednostní jízdy
                      </SelectItem>
                      <SelectItem value={UsageType.taxi}>Taxi</SelectItem>
                      <SelectItem value={UsageType.autopujcovna}>
                        Autopůjčovna
                      </SelectItem>
                      <SelectItem value={UsageType.autoskoly}>
                        Autoškoly
                      </SelectItem>
                      <SelectItem
                        value={UsageType.historickaVozidlaSeZvlastnimSPZ}
                      >
                        Historická vozidla se zvláštním SPZ
                      </SelectItem>
                    </SelectContent>
                  </Select>
                </div>
            ...
            <div
              className={`bg-white rounded-2xl shadow-lg border-2 p-8 transition-all duration-300 ${hasError("paymentFrequency") ? "border-red-500 animate-errorBlink" : "border-gray-200"}`}
              data-error={hasError("paymentFrequency")}
            >
              ...
              <div className="space-y-3">
                {hasError("paymentFrequency") && (
                  <div className="bg-red-50 border border-red-300 rounded-lg p-3 mb-4">
                    <p className="text-sm text-red-700 font-medium">
                      {getErrorMessage("paymentFrequency")}
                    </p>
                  </div>
                )}

                <Label className="text-sm font-medium text-gray-700 block mb-2">
                  Jak často chcete platit pojistné?{" "}
                  <span className="text-red-600">*</span>
                </Label>
                ...
                <div className="space-y-3">
                  <button
                    type="button"
                    onClick={() =>
                      setPaymentFrequency(PaymentFrequency.ctvrtletni)
                    }
                    className={`
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      setPaymentFrequency(PaymentFrequency.pololetni)
                    }
                    className={`
                      w-full px-5 py-3 rounded-xl font-semibold text-sm uppercase tracking-wide
                      transition-all duration-300 transform hover:scale-[1.02] active:scale-[0.98]
            ...
            <div className="text-xs text-gray-600 bg-gray-50 rounded-lg p-4 border border-gray-200 shadow-sm">
              ...
              <p className="leading-relaxed">
                Prohlašuji, že všechny údaje uvedené v tomto dotazníku jsou
                pravdivé a úplné. Jsem si vědom/a, že uvedení nepravdivých nebo
                neúplných údajů může mít za následek odmítnutí pojistného plnění
                nebo ukončení pojistné smlouvy.
              </p>
            </div>
          </div>
        </div>
