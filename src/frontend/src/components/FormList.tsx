          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              {newFormsCountNumber > 0 && (
                ...
                >
                  {newFormsCountNumber}{" "}
                  {newFormsCountNumber === 1
                    ? "nový"
                    : newFormsCountNumber < 5
                      ? "nové"
                      : "nových"}
                </Badge>
              )}
          ...
          {forms.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 text-lg">
                Zatím nebyly odeslány žádné formuláře
              </p>
          ...
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-2">
              ...
              {!form.viewedByAdmin && (
                <Badge
                  variant="destructive"
                  className="bg-red-600 text-white animate-fade-in"
                >
                  Nový
                </Badge>
              )}
            ...
            <div className="text-sm text-gray-600 space-y-1">
              <p>
                <strong>Email:</strong> {form.submitterEmail}
              </p>
              <p>
                <strong>Datum:</strong> {formatDate(form.createdAt)}{" "}
                <strong>Čas:</strong> {formatTime(form.createdAt)}
              </p>
            </div>
              ...
              <div className="mt-4 pt-4 border-t border-gray-200 space-y-4 animate-fade-in">
                ...
                >
                  ...
                  <div className="space-y-2">
                    ...
                    {renderBooleanField(
                      "Je firma",
                      form.provozovatel.isCompany,
                    )}
                    {form.provozovatel.isVatPayer &&
                      renderBooleanField(
                        "Plátce DPH",
                        form.provozovatel.isVatPayer,
                      )}
                  ...
                  <div className="space-y-2">
                    ...
                    {form.vozidlo.vehicleType.length > 0 && (
                      <p className="text-sm text-gray-700">
                        ...
                        {form.vozidlo.vehicleType.join(", ")}
                      </p>
                    )}
                    {renderField("Způsob užití", form.vozidlo.usageType)}
                    {renderField("Palivo", form.vozidlo.fuelType)}
                    {form.vozidlo.engineCapacity &&
                      renderField(
                        "Zdvihový objem",
                        `${form.vozidlo.engineCapacity} ccm`,
                      )}
                    {form.vozidlo.maxPower &&
                      renderField("Výkon", `${form.vozidlo.maxPower} kW`)}
                    {form.vozidlo.weight &&
                      renderField("Hmotnost", `${form.vozidlo.weight} kg`)}
                    {form.vozidlo.approximateValue &&
                      renderField(
                        "Přibližná hodnota",
                        `${form.vozidlo.approximateValue} Kč`,
                      )}
                    {renderBooleanField(
                      "Dovoz ze zahraničí",
                      form.vozidlo.importedFromAbroad,
                    )}
                    {renderBooleanField(
                      "Má technický průkaz",
                      form.vozidlo.hasTechnicalCertificate,
                    )}
                  </div>
                </div>
                ...
                >
                  ...
                  <div className="space-y-2">
                    {renderBooleanField(
                      "Povinné ručení",
                      form.insuranceOptions.povinneRuceni,
                    )}
                    {renderBooleanField(
                      "Havarijní pojištění",
                      form.insuranceOptions.havarijniPojisteni,
                    )}
                    {renderBooleanField(
                      "Pojištění skel",
                      form.insuranceOptions.pojisteniSkel,
                    )}
                    {renderBooleanField(
                      "Pojištění asistence",
                      form.insuranceOptions.pojisteniAsistence,
                    )}
                    {renderBooleanField(
                      "Pojištění přírodních rizik",
                      form.insuranceOptions.pojisteniPrirodnichRizik,
                    )}
                    {renderBooleanField(
                      "Pojištění GAP",
                      form.insuranceOptions.pojisteniGAP,
                    )}
                    {renderBooleanField(
                      "Pojištění náhradního vozidla",
                      form.insuranceOptions.pojisteniNahradnihoVozidla,
                    )}
                    {renderBooleanField(
                      "Úrazové připojištění",
                      form.insuranceOptions.urazovePripojisteni,
                    )}
            ...
            <AlertDialog>
              ...
              <AlertDialogContent className="bg-white">
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    Tato akce je nevratná. Formulář od {form.submitterName} bude
                    trvale odstraněn.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
